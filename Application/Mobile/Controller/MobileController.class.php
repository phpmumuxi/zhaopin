...
class MobileController extends FrontendController {
    ...
    public function is_weixin(){
        if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false){
            if(C('qscms_weixin_appid') && C('qscms_weixin_appsecret')){
                if(!$openid = session('openid')){
                    if(!$code = I('get.code','','trim')){
                        $code_url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.C('qscms_weixin_appid').'&redirect_uri='.$this->get_redirect_uri().'&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect';
                        !IS_AJAX && redirect($code_url);
                    }elseif($openid = $this->get_weixin_openid($code)){
                        $this->check_weixin_login($openid);
                    }else{
                        $this->visitor->logout();
                        //同步退出
