...
class CompanyController extends MobileController {
    ...
    protected function _global_variable() {
        ...
        // 短信必须验证
        if (C('qscms_login_com_audit_mobile') == 1 && !C('visitor.mobile') && !in_array(ACTION_NAME, array('com_security_tel', 'com_auth', 'com_info'))) {
            $this->error('您的手机未认证，认证后才能进行其他操作！', U('Company/com_security_tel'));
        }
        $this->_cominfo_flge();
        // 强制认证营业执照
    ...
    public function index() {
        ...
        $this->_config_seo(array('title' => '企业会员中心', 'header_title' => '企业会员中心'));
        $this->display();
    }

    public function com() {
        $this->_config_seo(array('title' => '企业管理', 'header_title' => '企业管理'));
        $this->display();
    }
    ...
    public function com_info() {
        ...
        if (!IS_POST) {
            ...
            $this->_config_seo(array('title' => '企业资料', 'header_title' => '企业资料'));
            $this->display();
        } else {//保存企业信息
            ...
            $this->ajaxReturn(1, $success, $r['data']);
        }
    }
    ...
    public function com_img() {
        ...
        $this->_config_seo(array('title' => '企业风采', 'header_title' => '企业风采'));
        $this->display();
    }
    ...
    public function com_auth() {
        ...
        $this->_config_seo(array('title' => '企业认证', 'header_title' => '企业认证'));
        $this->display();
    }
    ...
    public function jobs_add() {
        ...
        if (IS_POST && IS_AJAX) {
            ...
            $this->_config_seo(array('title' => '发布职位', 'header_title' => '发布职位'));
            $this->display();
        }
    }
    ...
    public function jobs_edit() {
        ...
        $this->assign('could_allowance', $could_allowance);
        $this->_config_seo(array('title' => '修改职位', 'header_title' => '修改职位'));
    ...
    public function jobs_list() {
        ...
        $this->assign('upper_limit', $upper_limit);
        $this->_config_seo(array('title' => '管理职位', 'header_title' => '管理职位'));
        $this->display();
    }
    ...
    public function jobs_apply() {
        ...
        $this->_config_seo(array('title' => '收到的简历', 'header_title' => '收到的简历'));
        $this->display();
    }
    ...
    public function resume_down() {
        ...
        $this->_config_seo(array('title' => '已下载的简历', 'header_title' => '已下载的简历'));
        $this->display();
    }
    ...
    public function resume_favorites() {
        ...
        $this->_config_seo(array('title' => '已收藏的简历', 'header_title' => '已收藏的简历'));
        $this->display();
    }
    ...
    public function pms_list() {
        if (I('get.type', 0, 'intval')) {
            ...
            $this->_config_seo(array('title' => '求职者咨询', 'header_title' => '求职者咨询'));
        } else {
            $settr = I('get.settr', 0, 'intval');
    ...
    public function pms_show() {
        ...
        $this->_config_seo(array('title' => '系统信息', 'header_title' => '系统信息'));
        $this->display();
    }
    ...
    public function jobfair_list() {
        ...
        $this->_config_seo(array('title' => '近期招聘会', 'header_title' => '近期招聘会'));
        $this->display();
    }
    ...
    public function jobfair_enact() {
        ...
        $this->_config_seo(array('title' => '我预定的展位', 'header_title' => '我预定的展位'));
        $this->display();
    }
    ...
    public function set_allowance_job() {
        ...
        $this->_config_seo(array('title' => '发布红包职位', 'header_title' => '发布红包职位'));
        $this->display();
    }
        ...
        if ($insertid) {
            ...
            if($setsqlarr['payment']=='wxpay')
            {
                if($this->is_weixin){
                    ...
                    $info = $model->find($insertid); 
                    $paysetarrs = $this->_get_pay_params($info);
                    $paysetarrs['h5_pay'] = 1;
                    $r = D('Payment')->pay($paysetarrs);
                    if($r['status']==1){
                        $this->ajaxReturn(1,'',$r['url']);
                    }else{
     ...
     * 获取支付相关参数
     */
    protected function _get_pay_params($info){
        $model = D('Allowance/AllowanceInfo');
        $paysetarrs['payFrom'] = 'wap';
        $paysetarrs['type'] = $info['payment'];
        $paysetarrs['ordsubject'] = $model->type_alias_cn[$info['type_alias']];
        $paysetarrs['ordbody'] = $model->type_alias_cn[$info['type_alias']];
        $paysetarrs['ordtotal_fee'] = $info['pay_amount'];
        $paysetarrs['oid'] = $info['oid'];
        return $paysetarrs;
    }
    ...
    public function allowance() {
        ...
        $this->_config_seo(array('title' => '我的打赏', 'header_title' => '我的打赏'));
        $this->display();
    }
    ...
    public function confirm_interview($record_id) {
        if (IS_POST) {
            ...
            $this->_config_seo(array('title' => '确认面试', 'header_title' => '确认面试'));
