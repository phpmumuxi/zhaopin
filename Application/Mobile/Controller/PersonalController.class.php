...
class PersonalController extends MobileController {
    ...
    protected function _global_variable() {
        // 帐号状态 为暂停
        if (!IS_AJAX && C('visitor.status') == 2 && !in_array(ACTION_NAME, array('index'))) {
            $this->error('您的账号处于暂停状态，请联系管理员设为正常后进行操作！', U('Personal/index'));
        }
        // 短信必须验证
        if (!IS_AJAX && !C('visitor.mobile') && !in_array(ACTION_NAME, array('resume_add'))) {
            //$this->error('您的手机未认证，认证后才能进行其他操作！', U('Personal/per_security_tel'));
            $this->redirect('members/mobile_auth');
        }
        $resume_count = D('Resume')->count_resume(array('uid' => C('visitor.uid')));//当前用户简历份数
        if (!$resume_count && !in_array(ACTION_NAME, array('resume_add','index','jobs_apply','attention_me','jobs_interview','jobs_favorites','del_favorites','attention_com','del_focus_company','per_security','per_security_tel','per_security_email','per_security_user','per_security_pwd','per_security_log','shield_company','shield_company_add','shield_company_del','feedback','per_binding','pms_list','pms_show','pms_consult_show','allowance'))) {
            IS_AJAX && $this->ajaxReturn(1,'您当前还没有简历，是否创建简历？',array('url'=>U('personal/resume_add')));
            $this->redirect('personal/resume_add');
        } elseif ($resume_count && in_array(ACTION_NAME, array('resume_add'))) {
            $this->redirect('personal/index');
        }
        if (!IS_AJAX && !S('personal_login_first_' . C('visitor.uid'))) {
    ...
    */
    public function index() {
        ...
        $this->_config_seo(array('title' => '个人会员中心', 'header_title' => '个人会员中心'));
        $this->display();
    }
    ...
    */
    public function resume_add() {
        ...
        if (IS_POST && IS_AJAX) {
            ...
            $this->_config_seo(array('title' => '创建简历', 'header_title' => '创建简历'));
            $this->display();
        }
    }
    ...
    */
    public function resume_edit() {
        ...
        $this->_config_seo(array('title' => '编辑简历', 'header_title' => '编辑简历'));
        $this->display();
    }
    ...
    */
    public function resume_edit_basis() {
        ...
        $category = D('Category')->get_category_cache();
        $birthdate_arr = range(date('Y') - 16, date('Y') - 66);
        $this->assign('education', $category['QS_education']);//最高学历
        $this->assign('experience', $category['QS_experience']);//工作经验
        $this->assign('birthdate_arr', $birthdate_arr);
        $this->_config_seo(array('title' => '基本信息', 'header_title' => '基本信息'));
        $this->display();
    }
    ...
    */
    public function resume_edit_intent() {
        ...
        $this->assign('jobs_nature', $category['QS_jobs_nature']);//工作性质
        $this->assign('wage', $category['QS_wage']);//期望薪资
        $this->_config_seo(array('title' => '求职意向', 'header_title' => '求职意向'));
        $this->display();
    }
    ...
    */
    public function resume_edit_description() {
        ...
        $this->_config_seo(array('title' => '自我描述', 'header_title' => '自我描述'));
        $this->display();
    }
    ...
    */
    public function resume_edit_edu() {
        ...
        $this->assign('education', $category['QS_education']);//最高学历
        $this->_config_seo(array('title' => '教育经历', 'header_title' => '教育经历'));
        $this->display();
    }
    ...
    */
    public function resume_edit_work() {
        ...
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '工作经历', 'header_title' => '工作经历'));
        $this->display();
    }
    ...
    */
    public function resume_edit_train() {
        ...
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '培训经历', 'header_title' => '培训经历'));
        $this->display();
    }
    /*
    *   简历-修改 - -项目
    */
    public function resume_edit_project() {
        $resume = $this->_is_resume();
        $id = I('request.id', 0, 'intval');
        if (IS_AJAX) {
            $setsqlarr['uid'] = C('visitor.uid');
            ...
            if (false === $resume = $this->_is_resume()) $this->ajaxReturn(0, '请先填写简历基本信息！');
            $setsqlarr['pid'] = $resume['id'];
            $project = M('ResumeProject')->where(array('pid' => $setsqlarr['pid'], 'uid' => $setsqlarr['uid']))->count();//获取项目经历数量
            if (count($training) >= 6) $this->ajaxReturn(0, '项目经历不能超过6条！');
            if ($id) {
                $setsqlarr['id'] = $id;
        ...
        $info = M('ResumeProject')->where(array('id' => $id, 'pid' => $resume['id'], 'uid' => C('visitor.uid')))->find();
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '项目经历', 'header_title' => '项目经历'));
        $this->display();
    }
    ...
    */
    public function resume_edit_certificate() {
        ...
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '获得证书', 'header_title' => '获得证书'));
        $this->display();
    }
    ...
    */
    public function resume_edit_lang() {
        ...
        $this->_config_seo(array('title' => '语言能力', 'header_title' => '语言能力'));
        $this->display();
    }
    ...
    */
    public function resume_edit_speciality() {
        ...
        $this->_config_seo(array('title' => '特长标签', 'header_title' => '特长标签'));
        $this->display();
    }
    ...
    */
    public function resume_edit_img() {
        ...
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '照片作品', 'header_title' => '照片作品'));
        $this->display();
    }
    ...
    */
    public function resume_replenish() {
        ...
        $info && $resume['img'] = 1;
        $this->assign('resume', $resume);
        $this->_config_seo(array('title' => '完善简历', 'header_title' => '完善简历'));
        $this->display();
    }
    ...
    */
    ...
    public function jobs_interview() {
        ...
        $this->_config_seo(array('title' => '面试邀请', 'header_title' => '面试邀请'));
        $this->display();
    }
    // 删除面试邀请
    ...
    public function attention_me() {
        ...
        $this->_config_seo(array('title' => '谁看过我', 'header_title' => '谁看过我'));
        $this->display();
    }
    ...
    public function jobs_apply() {
        ...
        $this->_config_seo(array('title' => '求职反馈', 'header_title' => '求职反馈'));
        $this->display();
    }
    ...
    public function jobs_favorites() {
        ...
        $this->_config_seo(array('title' => '职位收藏夹', 'header_title' => '职位收藏夹'));
        $this->display();
    }
    ...
    public function attention_com() {
        ...
        $this->_config_seo(array('title' => '关注的企业', 'header_title' => '关注的企业'));
        $this->display();
    }
    ...
    public function per_security() {
        $this->_config_seo(array('title' => '帐号安全', 'header_title' => '帐号安全'));
        $this->display();
    }
    ...
    public function per_security_email() {
        $this->_config_seo(array('title' => '邮箱认证', 'header_title' => '邮箱认证'));
        $this->display();
    }

    public function per_security_user() {
        $this->_config_seo(array('title' => '修改用户名', 'header_title' => '修改用户名'));
        $this->display();
    }

    public function per_security_pwd() {
        $this->_config_seo(array('title' => '修改密码', 'header_title' => '修改密码'));
        $this->display();
    }
    ...
    public function per_security_log() {
        ...
        $this->_config_seo(array('title' => '会员登录日志', 'header_title' => '会员登录日志'));
        $this->display();
    }
    ...
    public function shield_company() {
        ...
        $this->_config_seo(array('title' => '屏蔽企业', 'header_title' => '屏蔽企业'));
        $this->display();
    }
    /**
    ...
    public function feedback() {
        ...
        $this->_config_seo(array('title' => '意见反馈', 'header_title' => '意见反馈'));
        $this->display();
    }
    ...
    public function per_binding() {
        ...
        $this->_config_seo(array('title' => '账号绑定', 'header_title' => '账号绑定'));
        $this->display();
    }
    ...
    public function pms_list() {
        if (I('get.type', 0, 'intval')) {//留言咨询
            ...
            $this->_config_seo(array('title' => '消息提醒', 'header_title' => '消息提醒'));
        } else {
            $settr = I('get.settr', 0, 'intval');
    ...
    public function pms_show() {
        ...
        $this->_config_seo(array('title' => '系统消息', 'header_title' => '系统消息'));
        $this->display();
    }
    ...
    public function pms_consult_show() {
        ...
        $this->_config_seo(array('title' => '留言咨询', 'header_title' => '留言咨询'));
        $this->display();
    }
    ...
    public function allowance() {
        ...
        $this->_config_seo(array('title' => '我的红包', 'header_title' => '我的红包'));
        $this->display();
    }
    ...
    public function allowance_detail($id) {
        ...
        $this->_config_seo(array('title' => '打赏详情', 'header_title' => '打赏详情'));
        $this->display();
    }
    ...
    public function ever_interview() {
        ...
        if (IS_POST) {
            ...
            $this->_config_seo(array('title' => '我已面试', 'header_title' => '我已面试'));
    ...
    public function ever_entry() {
        ...
        if (IS_POST) {
            ...
            $this->assign('record_id', $record_id);
            $this->_config_seo(array('title' => '我已入职', 'header_title' => '我已入职'));
    ...
    public function allowance_interview_detail($record_id) {
        ...
        $this->assign('info', $info);
        $this->_config_seo(array('title' => '面试详情', 'header_title' => '面试详情'));
        $this->display();
    }
