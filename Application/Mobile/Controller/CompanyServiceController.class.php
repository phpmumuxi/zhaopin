...
class CompanyServiceController extends CompanyController{
    ...
    public function index(){
		...
		$this->_config_seo(array('title'=>'我的套餐','header_title'=>'我的套餐'));
    ...
    public function setmeal_detail(){
		...
		$this->_config_seo(array('title'=>'套餐使用明细','header_title'=>'套餐使用明细'));
    ...
    public function gold(){
		...
		$this->_config_seo(array('title'=>'我的'.C('qscms_points_byname'),'header_title'=>'我的'.C('qscms_points_byname')));
    ...
    public function gold_log(){
    	...
    	$this->_config_seo(array('title'=>C('qscms_points_byname').'使用明细','header_title'=>C('qscms_points_byname').'使用明细'));
    ...
    public function gold_task(){
    	...
    	$this->_config_seo(array('title'=>'做任务','header_title'=>'做任务'));
    ...
    public function order_list(){
    	...
    	$this->_config_seo(array('title'=>'我的订单','header_title'=>'我的订单'));
    ...
    public function order_detail($order_id){
    	...
    	$this->assign('my_setmeal',$this->my_setmeal);
    	$this->assign('order_type',D('Order')->order_type);
    	$this->_config_seo(array('title'=>'订单详情','header_title'=>'订单详情'));
	...
	public function alipay_submit(){
        ...
        if($this->my_points<$order['pay_points']){
            $this->error('你的'.C('qscms_points_byname').'不足，无法完成支付，请重新下单！');
        }
    ...
    public function service(){
    	...
    	$this->_config_seo(array('title'=>'增值服务','header_title'=>'增值服务'));
    ...
    public function service_resume(){
    	...
    	if(IS_POST){
	    	...
	    	$this->_config_seo(array('title'=>'购买简历包','header_title'=>'购买简历包'));
    ...
    public function service_sms(){
    	...
    	if(IS_POST){
	    	...
	    	$this->_config_seo(array('title'=>'购买短信包','header_title'=>'购买短信包'));
    ...
    public function service_stick(){
    	...
    	if(IS_POST){
	    	...
	    	$this->_config_seo(array('title'=>'职位置顶','header_title'=>'职位置顶'));
    ...
    public function service_emergency(){
    	...
    	if(IS_POST){
	    	...
	    	$this->_config_seo(array('title'=>'职位紧急','header_title'=>'职位紧急'));
    ...
    public function service_refresh(){
    	...
    	if(IS_POST){
	    	...
	    	$this->_config_seo(array('title'=>'职位智能刷新','header_title'=>'职位智能刷新'));
    ...
    public function gold_add(){
    	...
    	if(IS_POST){
        	...
        	$this->assign('payment_rate',C('qscms_payment_rate'));
    		$this->assign('my_points',$this->my_points);
	    	$this->_config_seo(array('title'=>'充值'.C('qscms_points_byname'),'header_title'=>'充值'.C('qscms_points_byname')));
    ...
    public function setmeal_add(){
    	...
    	if(IS_POST){
    		...
    		$this->assign('my_points',$this->my_points);
			$this->assign('my_setmeal',$this->my_setmeal);
	    	$this->_config_seo(array('title'=>'升级套餐','header_title'=>'升级套餐'));
    ...
    protected function _get_service_list($cat){
		...
		foreach ($increment_arr as $key => $value) {
			...
			$increment_arr[$key]['vip_price'] = intval($vip_discount)>0?round($value['price']*$vip_discount/10,1):$value['service_price'];
			//换算积分
