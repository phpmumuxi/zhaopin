...
class resume_listTag {
    ...
    public function __construct($options) {
        ...
        if($map) $this->where[] = "MATCH (`key`) AGAINST ('".implode(' ',$map)."' IN BOOLEAN MODE)";
		...
		if($this->params['page']){
            if($this->params['readresume']){
                ...
                if(C('qscms_resumesearch_mod')){
                    ...
                    // var_dump($model->Table($db_pre.$this->mod.' r')->where($this->where)->join($this->join)->fetchSql()->count('id'));
