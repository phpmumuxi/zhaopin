...
class ResumeWorkModel extends Model{
	protected $_validate = array(
		...
		array('uid,pid,startyear,startmonth,endyear,endmonth,todate','identicalEnum','',1,'callback'),
	);
	/**
	 * [工作经验结束时间验证]
	 * 工作经验结束年份+月份、至今必选一项
	...
	*/
	public function add_resume_work($data,$user)
	{
		...
		//写入会员日志
		write_members_log($user,2020,$insert_id);
		$resume_mod = D('Resume');
	...
	*/
	public function save_resume_work($data,$user)
	{
		...
		//写入会员日志
		write_members_log($user,2021,$data['pid']);
