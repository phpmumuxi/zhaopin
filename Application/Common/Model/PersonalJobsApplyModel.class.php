...
class PersonalJobsApplyModel extends Model{
	...
	*/
	...
	public function jobs_apply_add($jids,$user,$rid){
		...
		$field = 'id,uid,fullname,title,district_cn as district,experience,experience_cn,education,intention_jobs as jobs,audit';
        if($rid){
		...
		foreach($jids as $val){
			...
			if(false !== $this->create($jobs[$val])){
				if(false !== $insertid = $this->add()){
					...
					$replac_pms['jobs_url']=url_rewrite('QS_jobsshow',array('id'=>$jobs[$val]['jobs_id']));
					$replac_pms['resume_url']=url_rewrite('QS_resumeshow',array('id'=>$resume['id']));
					...
					if(C('qscms_sms_open')==1 && $sms['set_applyjobs']== 1 && $jobs_contact['notify_mobile'] == 1){
						...
						if($send_sms==true){
							...
							$r = $sms->sendSMS($usermobile['mobile'],array($resume['fullname'],$jobs_name),"457455");						
							if($r===true){
