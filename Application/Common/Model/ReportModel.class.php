...
class ReportModel extends Model{
	...
	public function add_report($data,$user){
		...
		if($jobs_info){
			...
			if($insert){
				/* 会员日志 */
				write_members_log($user,2042,$data['jobs_id']);
				//检测加入黑名单
		...
		{	
			...
			if($return>0){
				...
				foreach ($result as $key => $list) {
					...
					C('apply.Subsite') && $subsite_id = $subsite_district[$jobsinfo['tdistrict']]?:($subsite_district[$jobsinfo['sdistrict']]?:$subsite_district[$jobsinfo['district']]);
					$jobsurl=url_rewrite('QS_jobsshow',array('id'=>$list['jobs_id']),$subsite_id);
					if(!$jobsinfo)
					{
						continue;
					}
					...
					if($audit==2)
					{
						$r = D('TaskLog')->do_task($user_info,13);
						...
						D('Pms')->write_pmsnotice($user_info_com['uid'],$user_info_com['username'],$message_c);
					}
					else
					{
