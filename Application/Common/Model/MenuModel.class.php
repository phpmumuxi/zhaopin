...
class MenuModel extends Model {
    ...
    public function sub_menu_cache($pid = 0){
        ...
        $where = array('role_id'=>$_SESSION['admin']['role_id'],'m.pid'=>$pid,'m.display'=>1);
    ...
    public function auth_menu_cache(){
        $db_pre = C('DB_PREFIX');
        $where = array('role_id'=>$_SESSION['admin']['role_id'],'m.display'=>1);
        !APP_DEVELOPER && $where['m.mods'] = array('neq',1);
    ...
    public function auth_cache(){
        $this->group_auth();
        $db_pre = C('DB_PREFIX');
        $where = array('role_id'=>$_SESSION['admin']['role_id']);
        !APP_DEVELOPER && $where['m.mods'] = array('neq',1);
    ...
    protected function _after_delete($data,$options){
        $options['where']['id'][1] && $this->where(array('pid'=>array('in',$options['where']['id'][1])))->delete();
        $this->_before_write();
    }
}