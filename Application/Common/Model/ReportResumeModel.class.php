...
class ReportResumeModel extends Model{
	...
	public function add_report($data,$user){
		...
		if($resume_info){
			...
			$data['addtime'] = time();
			$data['content'] = '';
			C('SUBSITE_VAL.s_id') && $data['subsite_id'] = C('SUBSITE_VAL.s_id');
			$insert = $this->add($data);
			if($insert){
				/* 会员日志 */
				write_members_log($user,1032,$data['resume_id']);
				//检测加入黑名单
		...
		{	
			...
			if($return>0){
				...
				foreach ($result as $key => $list) {
					...
					if ($audit==2)
					{
						$r = D('TaskLog')->do_task($user_info,21);
						
						$msg_p = "，奖励".$r['data'].C('qscms_points_byname')."，感谢您对".C('qscms_site_name')."的支持！";
						$message="您于".$timestring."举报的简历：【<a href=\"{$resumeurl}\" target=\"_blank\">{$list['title']}</a>】，经平台核实情况属实".$msg;
						D('Pms')->write_pmsnotice($user_info['uid'],$user_info['username'],$message);

						$message_c="您发布的简历【".$resumeinfo['title']."】于".$timestring."被举报，经平台核实情况属实，请尽快处理，如再有此类情况发生将作封号处理！";
						D('Pms')->write_pmsnotice($user_info_per['uid'],$user_info_per['username'],$message_c);
					}
					else
					{
						$message="您于".$timestring."举报的简历：【<a href=\"{$resumeurl}\" target=\"_blank\">{$list['title']}</a>】，经平台核实情况不属实";
						D('Pms')->write_pmsnotice($user_info['uid'],$user_info['username'],$message);
					}
				 }
			}
