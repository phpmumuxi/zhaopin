...
class BackendController extends BaseController{
    ...
    public function index() {
        ...
        if ($map['qs_order.payment_cn']=='不限'){
            unset($map['qs_order.payment']);
            unset($map['qs_order.payment_cn']);
        }
        $mod = D($this->_name);
...
//   var_dump($_POST);exit;
    ...
    public function edit(){
        ...
        if (IS_POST) {
            ...
            if (false !== $mod->save($data)) {
                ...
                //统一写日志
                $this->admin_write_log_unify($data[$pk]);
    ...
    protected function check_priv(){
        ...
        if(!session('admin') && !in_array(ACTION_NAME, array('login','verify_code'))) exit('<script type="text/javascript">top.location="'.U('Admin/index/login').'";</script>');
