...
class get_dir_file{
    ...
    public function getDownloadPath($mod,$username,$password,$domain){
        if(!$mod) return '请选择要升级的应用！';
        foreach ($mod as $key => $val) {
            $mods[] = $val['alias'].'|'.$val['version'];
        }
        $url = $this->download_url . '&module=' . implode(',',$mods) . '&username='.$username.'&password='.$password.'&domain='.$domain;
