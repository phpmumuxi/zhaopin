...
class user_visitor {
    ...
    public function __construct() {        
        ...
        if (session('?'.$this->_session)) {
            ...
            $field = 'uid,utype,username,email,mobile,last_login_ip';
    ...
    public function login($uid,$remember = true) {
        ...
        // $user_unlogin_remind_days = $utype==1?30:30;
        ...
        $field = 'uid,utype,username,email,mobile,password,last_login_time';
        C('apply.Subsite') && $field.=',subsite_id';
        ...
        write_members_log($user_info,1001,'','','',$log_source);
        //记录路由
    ...
    public function record_route_group($user){
        ...
        $logincount = D('MembersLog')->where(array('log_uid'=>$user['uid'],'log_type'=>1001,'log_addtime'=>array('between',array(strtotime('today'),strtotime('tomorrow')))))->count();
    ...
    public function logout() {
		...
		//var_dump( session($this->_session));exit;
    }
}