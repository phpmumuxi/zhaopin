...
!function($) {
  ...
  $('.J_resuletitle_trade').live('click', function() {
	...
	// 点击显示地区分类
	$('#district_cn').live('click', function() {
		var titleValue = $(this).data('title');
		var multipleValue = eval($(this).data('multiple'));
		var maxNumValue = eval($(this).data('maxnum'));
		var widthValue = eval($(this).data('width'));
		...
		if (classifyModel) { // 二级分类
								...
								if (QS_city[combinationArray[1]]) {
									if (QS_city[combinationArray[1]].split('`')) {
										...
										if (!addJob) {
											...
											'<li>',
												'<label>',
													'<input class="J_list_city J_list_city_nolimit" type="checkbox" data-code="' + combinationArray[0] + '.' + combinationArray[1] + '.0" data-title="' + combinationArray[2] + '"> ',
												'不限</label>',
											'</li>'
											].join('');
										}
										for (var j = 0; j < city22Array.length; j++) {
											if (city22Array[j].split(',')) {
												...
												htmlCity += [
													'<li>',
														'<label>',
															'<input class="J_list_city" type="checkbox" data-code="' + combinationArray[0] + '.' + combinationArray[1] + '.' + jArray[0] + '" data-title="' + jArray[1] + '"> ',
														'' + jArray[1] + '</label>',
													'</li>'
												].join('');
											};
								...
								if (QS_city[combinationArray[1]]) {
									if (QS_city[combinationArray[1]].split('`')) {
										...
										if (!addJob) {
											htmlCity += [
											'<li>',
												'<label class="J_list_city J_list_city_nolimit" type="checkbox" data-code="' + combinationArray[0] + '.' + combinationArray[1] + '.0" data-title="' + combinationArray[2] + '">',
												'不限</label>',
											'</li>'
											].join('');
										}
										for (var j = 0; j < city22Array.length; j++) {
											if (city22Array[j].split(',')) {
												...
												htmlCity += [
													'<li>',
														'<label class="J_list_city" type="checkbox" data-code="' + combinationArray[0] + '.' + combinationArray[1] + '.' + jArray[0] + '" data-title="' + jArray[1] + '">',
														'' + jArray[1] + '</label>',
													'</li>'
												].join('');
											};
		    ...
		    $('.J_list_city').die().live('click', function() {
		    	if (multipleValue) {
					...
					$('.J_resultcode_city').val(code);
					$('.J_resuletitle_city').val(title + '/' + title1 + '/' + title2);
					$('.J_resuletitle_city').attr('title', title);
					$('.modal_backdrop').remove();
	 				$('.modal').remove();
		    	}
