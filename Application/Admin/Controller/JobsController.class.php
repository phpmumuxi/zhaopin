...
class JobsController extends BackendController{
    ...
    public function index(){
        $jobtype=I('get.jobtype',0,'intval');
    ...
    public function delete_jobs(){
        ...
        {
        admin_write_log("删除职位id为".$id."的职位", C('visitor.username'),3);
    ...
    public function set_audit(){
        ...
        $result = $this->_mod->admin_edit_jobs_audit($id,$audit,$reason,C('visitor.username'));
        if($result){
     ...
     * 改变职位索引key
     */
    public function setJobsKey($id)
    {
        !is_array($id) && $id=array($id);
        foreach($id as $val){
            $res = D('Jobs')->field('jobs_name,companyname,category_cn,district_cn,contents')->where(array('id'=>$val))->find();
            if($res){                
                // key 分词
                $key = $res['jobs_name'].$res['companyname'].$res['category_cn'].$res['district_cn'].$res['contents'];
                $key = implode(' ',get_tags($key,100));
                M('Jobs')->where(array('id'=>$val))->setField('key',$key);
                M('JobsSearchKey')->where(array('id'=>$val))->setField('key',$key);
            }
        }
    }
    /**
    ...
    protected function _update_jobs($data){
        ...
        $districtcategory = $data['districtcategory'];
        $districtcategory_arr = explode(".", $districtcategory);
        $data['district']= $districtcategory_arr[0];
        $data['sdistrict']= $districtcategory_arr[1];
        $data['tdistrict']= $districtcategory_arr[2];
        $rst = D('Jobs')->admin_edit_jobs($data,$userinfo);
        if($rst['state']==0){
            $this->error($rst['error']);
        }else{
            $this->success('修改职位成功！',U('Jobs/index'));
        }
    }
}
?>