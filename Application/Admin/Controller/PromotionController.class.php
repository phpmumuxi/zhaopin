...
class PromotionController extends BackendController{
    ...
    public function _after_insert($id,$data){
        ...
        admin_write_log("添加推广：{$promotion},职位ID：{$setsqlarr['cp_jobid']}", C('visitor.username'),3);
    }
    ...
    public function ajax_get_jobs(){
        ...
        foreach ($result as $key => $value) {
            ...
            $value['jobs_url']=url_rewrite('QS_jobsshow',array('id'=>$value['id']),$subsite_id);
            $info[]=$value['id']."%%%".$value['jobs_name']."%%%".$value['jobs_url']."%%%".$value['companyname']."%%%".$value['company_url']."%%%".$value['addtime']."%%%".$value['deadline']."%%%".$value['refreshtime']."%%%".$subsite_id;
        }
        if (!empty($info))
        {
        exit(implode('@@@',$info));
    ...
    public function _after_select($info){
        ...
        C('apply.Subsite') && $subsite_id = $subsite_district[$jobs['tdistrict']]?:($subsite_district[$jobs['sdistrict']]?:$subsite_district[$jobs['district']]);
        $jobs['jobs_url']=url_rewrite('QS_jobsshow',array('id'=>$jobs['id']),$subsite_id);
