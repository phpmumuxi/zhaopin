...
class PersonalController extends BackendController{
     ...
     * 改变简历索引key
     */
    public function setResumeKey($ids)
    {   
        !is_array($ids) && $ids=array($ids);
        foreach($ids as $val){
            $resume = D('Resume')->field('id,uid,intention_jobs,education_cn,specialty,key_full,key_precise')->where(array('id'=>$val))->find();
            if($resume){
                $key_full = $resume['intention_jobs'].$resume['education_cn'].$resume['specialty'];
                $key_precise = $resume['intention_jobs'];
                $resume_education=M('ResumeEducation')->field('school,speciality')->where(array('pid'=>$resume['id'],'uid'=>$resume['uid']))->select();
                if (!empty($resume_education)){
                    foreach($resume_education as $li){
                        $key_full .= $li['school'].$li['speciality'];                    
                    }
                }
                $resume_work=M('ResumeWork')->field('companyname,jobs,achievements')->where(array('pid'=>$resume['id'],'uid'=>$resume['uid']))->select();
                if (!empty($resume_work)){
                    foreach($resume_work as $li){
                        $key_full .= $li['companyname'].$li['jobs'].$li['achievements'];
                        $key_precise .= $li['jobs'];
                    }            
                }
                $resume_training=M('ResumeTraining')->field('agency,course,description')->where(array('pid'=>$resume['id'],'uid'=>$resume['uid']))->select();
                if (!empty($resume_training)){
                    foreach($resume_training as $li){
                        $key_full .= $li['agency'].$li['course'].$li['description'];
                    }                
                }
                $resume_language=M('ResumeLanguage')->field('language_cn')->where(array('pid'=>$resume['id'],'uid'=>$resume['uid']))->select();
                if(!empty($resume_language)){
                    foreach($resume_language as $li){
                        $key_full .= $li['language_cn'];
                    }
                }
                $key_full = $resume['key_full']." ".implode(' ',get_tags($key_full,100));
                $key_precise = $resume['key_precise']." ".implode(' ',get_tags($key_precise,100));
                M('Resume')->where(array('id'=>$val))->save(array('key_full'=>$key_full,'key_precise'=>$key_precise));
                M('ResumeSearchFull')->where(array('id'=>$resume['id'],'uid'=>$resume['uid']))->setField('key',$key_full);
                M('ResumeSearchPrecise')->where(array('pid'=>$resume['id'],'uid'=>$resume['uid']))->setField('key',$key_precise);
            }
        }
    }
    /**
