...
class CallbackController extends FrontendController{
    ...
    public function oauth(){
        ...
        if(false === $url) $this->error($oauth->getError(),__ROOT__);
        redirect($url);
    }
    /**
     * [binding 微信绑定]
    ...
    public function weixin_reg(){
        ...
        if(false !== F('/weixin/'.($event_key%10).'/'.$event_key)){
            ...
            $data['utype'] = I('get.utype',0,'intval');
            $data['reg_type'] = 3;
            $reg = \Common\qscmslib\weixin::reg($openid);
            if($reg['state']){
                $userinfo = $reg['data'];
                $result['keyid'] = $userinfo['openid'];
                $result['keyname'] = $data['username'] = $userinfo['nickname'];
                $userinfo = serialize($result);
                $passport = $this->_user_server();
                if(false === $data = $passport->register($data)) return array('state'=>0,'tip'=>$passport->get_error());
                M('MembersBind')->add(array('uid' => $data['uid'],'type' => 'weixin','keyid' => $openid,'info' => $userinfo));
                $members_mod = D('Members');
                $members_mod->user_register($data);
                if(false !== $this->visitor->login($data['uid'])){
                    redirect(build_mobile_url(array('c'=>'Members','a'=>'login')));
                }
                session('reg_scene_id',null);
                F('/weixin/'.($event_key%10).'/'.$event_key,null);
            }
        }
    }
    /**
    ...
    */
    ...
    {   
        $arr = $_POST;
        $this->writeLog('支付宝>>'.var_export($arr,true));
        $pay = new \Common\qscmslib\pay('alipay');
        // $verify_result = $pay->checkSign($arr);
        // if($verify_result) {
            // $this->writeLog('---支付宝ok>>');
            //验证成功
    ...
    public function alipay_notify_url0()
    {   
        $pay = new \Common\qscmslib\pay('alipay');
    ...
    public function alipay_return_url0()
    {
        $pay = new \Common\qscmslib\pay('alipay');
