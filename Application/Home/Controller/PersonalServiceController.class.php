    ...
    */
     ...
     */
     public function task(){
        ...
        $this->assign('task',D('Task')->get_task_cache());
        $this->assign('mypoints',$this->my_points);
        $this->assign('points_count',D('TaskLog')->count_task_points($this->uid,C('visitor.utype')));
    ...
    public function increment_add($cat=''){
        ...
        switch($cat){
            case 'stick':
                ...
                $resume = D('Resume')->get_resume_list(array('where'=>array('uid'=>$this->uid,'display'=>1),'field'=>'id,title,audit,stick','_audit'=>1));
                $this->assign('resume',$resume);
                $tpl_name = 'increment_add_stick';break;
            case 'tag':
                ...
                $resume = D('Resume')->get_resume_list(array('where'=>array('uid'=>$this->uid,'display'=>1),'field'=>'id,title,audit,strong_tag','_audit'=>1));
                $this->assign('resume',$resume);
                $tpl_name = 'increment_add_tag';break;
            case 'tpl':
                ...
                $tpl_name = 'increment_add_tpl';break;
        }
    ...
    public function _increment_add_save_stick(){
        ...
        if($pay_type=='points'){
            ...
            if($rst['state']==1){
                ...
                D('Order')->add_order(C('visitor'),$oid,$order_pay_type,$service_need_cash,0,$service_need_points,$htype_cn,'points',C('qscms_points_byname').'支付',$description,$this->timestamp,2,0,0,$this->timestamp,serialize(array('days'=>$days)));
               /* 会员日志 */
                ...
                if($p_rst){
                    /* 会员日志 */
                    write_members_log(C('visitor'),9002,$htype_cn,C('qscms_points_byname').'兑换');
    ...
    public function _increment_add_save_tpl(){
        ...
        if($pay_type=='points'){
            ...
            if($rst['state']==1){
                $oid= "P-".date('ymd',time())."-".date('His',time());//订单号
                $description = '购买服务：'.$htype_cn.';'.C('qscms_points_byname').'支付'.$service_need_points.C('qscms_points_byname');
                D('Order')->add_order(C('visitor'),$oid,$order_pay_type,$service_need_cash,0,$service_need_points,$htype_cn,'points',C('qscms_points_byname').'支付',$description,$this->timestamp,2,0,0,$this->timestamp);
    ...
    protected function _call_cash_pay($htype_cn,$order_pay_type,$payment_name='',$amount='0.0',$is_deductible=0,$deductible=0,$params=''){
        ...
        D('Order')->add_order(C('visitor'),$paysetarr['oid'],$order_pay_type,$amount,$paysetarr['ordtotal_fee'],$deductible,$htype_cn,$payment_name,$paymenttpye['byname'],$description,$this->timestamp,1,0,0,0,$params);
