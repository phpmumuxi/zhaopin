...
class MembersController extends FrontendController{
	public function _initialize() {
        ...
        if(!$this->visitor->is_login) {
            ...
            !IS_AJAX && !in_array(ACTION_NAME, array('logout','varify_email')) && $this->redirect($urls[C('visitor.utype')],array('uid'=>$this->visitor->info['uid']));
        }
	}
	...
	public function login() {
        ...
        if(IS_AJAX && IS_POST){
            ...
            if($mobile = I('post.mobile','','trim')){
                ...
                if(time()>$smsVerify['time']+60) $this->ajaxReturn(0,'验证码过期！');//验证码过期
            ...
            $this->assign('verify_userlogin',$this->check_captcha_open(C('qscms_captcha_config.user_login')));
            $this->display();
        }
    }
    /**
    ...
    public function register(){
        ...
        if(IS_POST && IS_AJAX){
            ...
            if (C('qscms_captcha_open')==1 && C('qscms_captcha_config.varify_reg')){
                if(true !== $reg = \Common\qscmslib\captcha::verify()) $this->ajaxReturn(0,$reg);
            }
    ...
    public function reg_send_sms(){
        ...
        if (true === $reg = $sms->sendSMS($mobile,array("{$rand}"),"193822")){ 
    ...
    public function user_getpass(){
        if(IS_POST){
            ...
            if($type == 1){
                ...
                if(time()>$smsVerify['time']+60) $this->error('验证码过期！');//验证码过期
                $vcode_sms = I('post.mobile_vcode',0,'intval');
                $mobile_rand=substr(md5($vcode_sms), 8,16);
    ...
    public function oauth_reg(){
        ...
        cookie('members_bind_info', NULL);//清理绑定COOKIE
        $urls = array('1'=>'company/index','2'=>'personal/index');
        $this->redirect($urls[$this->visitor->info['utype']],array('uid'=>$this->visitor->info['uid']));
    }
    /**
