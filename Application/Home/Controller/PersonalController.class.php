...
use Common\ORG\PHPExcel\PHPExcel;
...
class PersonalController extends FrontendController{
    ...
    //excel添加用户和简历
    public function adduserandresum(){
    ...
    $nature = [
    	...
    	//echo $dirname;exit;
    			...
    			//$objReader = new \PHPExcel_IOFactory::createReader('Excel5');
    			...
    			{
    				//用户信息
    				$userdata["mobile"] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("H".$i)->getValue());
    				$userdata["email"] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("A".$i)->getValue());
    				$userdata["username"] = "my".$userdata["mobile"];
    				$userdata["password"] = "123456";//默认密码123456
    				$userdata["repassword"] = "123456";//默认密码确认123456
    				$userdata["utype"] = "2";//默认为个人
    				//用户简历信息
    				$resumedata["display_name"] = "1";//默认为显示
    				$resumedata["title"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("B".$i)->getValue());
    				$resumedata["fullname"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("C".$i)->getValue());
    				$resumedata["sex"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("D".$i)->getValue());
    				$resumedata["residence"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("E".$i)->getValue());
    				$resumedata["email"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("F".$i)->getValue());
    				$resumedata["birthdate"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("G".$i)->getValue());
    				$resumedata["telephone"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("H".$i)->getValue());
    				$resumedata["education"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("I".$i)->getValue());
    				$resumedata["experience"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("J".$i)->getValue());
    				$resumedata["major"] =0;
    				$resumedata["email_notify"] =0;
    				$resumedata["height"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("K".$i)->getValue());
    				$resumedata["householdaddress"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("L".$i)->getValue());
    				$resumedata["marriage"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("M".$i)->getValue());
    				$resumedata["qq"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("N".$i)->getValue());
    				$resumedata["weixin"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("O".$i)->getValue());
    				$resumedata["nature"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("P".$i)->getValue());
    				$resumedata["current"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("Q".$i)->getValue());
    				$resumedata["wage"] =(string)trimall($objPHPExcel->getActiveSheet()->getCell("R".$i)->getValue());
    				$specialty =(string)trimall($objPHPExcel->getActiveSheet()->getCell("S".$i)->getValue());
    				$resumedata["intention_jobs"] =0;
    				$resumedata["intention_jobs_id"] ="1.12.71,1.12.72";
    				$resumedata["trade"] ="5,8,14";
    				$resumedata["district"] ="11.121.0";
					if(!$userdata["mobile"]){
						//echo 1;exit;

						$adduserinfo=M("members")->where(["uid" => session("lastresumeUid")])->find();
						//教育经历
						$setsqlarr['uid'] = session("lastresumeUid");
						$setsqlarr['pid'] = session("lastresumeId");
						
						$setsqlarr['school'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("T".$i)->getValue());
						$setsqlarr['speciality'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("U".$i)->getValue());
						$setsqlarr['education'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("V".$i)->getValue());
						$setsqlarr['startyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("W".$i)->getValue());
						$setsqlarr['startmonth'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("X".$i)->getValue());
						$setsqlarr['endyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("Y".$i)->getValue());
						$setsqlarr['endmonth'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("Z".$i)->getValue());
						$setsqlarr["education_cn"] = $education[$setsqlarr['education']];
						
						$reg = D('ResumeEducation')->add_resume_education($setsqlarr,$adduserinfo);

						//工作经历
						$worksqlarr['uid'] = session("lastresumeUid");
						$worksqlarr['pid'] = session("lastresumeId");
						$worksqlarr['companyname'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AA".$i)->getValue());
						$worksqlarr['jobs'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AB".$i)->getValue());
						$worksqlarr['startyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AC".$i)->getValue());
						$worksqlarr['startmonth'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AD".$i)->getValue());
						$worksqlarr['endyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AE".$i)->getValue());
						$worksqlarr['endmonth'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AF".$i)->getValue());
						$worksqlarr['achievements'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AG".$i)->getValue());
						$worksqlarr['todate'] = 0; // 至今
						
						$regwork=D('ResumeWork')->add_resume_work($worksqlarr,$adduserinfo);
			
						
					}else{
					
    				$userinfo = $this->addallmember($userdata);
    				$adduserinfo=M("members")->where(["mobile" => $userdata["mobile"]])->find();
    				//var_dump($user_id);exit;
    				if($userinfo['success']){

    					$resumeResult = $this->resume_addall($resumedata,$adduserinfo);
    					if($resumeResult["success"]){
    						//自我描述
    						if($specialty)
    							$rst=D('Resume')->save_resume(array('specialty'=>$specialty),$resumeResult['id'],$adduserinfo);
    						//教育经历
    						$setsqlarr['uid'] = $adduserinfo["uid"];
    						$setsqlarr['pid'] = $resumeResult["id"];
		    				$setsqlarr['school'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("T".$i)->getValue());
		    				$setsqlarr['speciality'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("U".$i)->getValue());
		    				$setsqlarr['education'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("V".$i)->getValue());
		    				$setsqlarr['startyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("W".$i)->getValue());
    						...
    						$setsqlarr["education_cn"] = $education[$setsqlarr['education']];
		    				$reg = D('ResumeEducation')->add_resume_education($setsqlarr,$adduserinfo);
		    				session("lastresumeId",$setsqlarr['pid']);
		    				session("lastresumeUid",$setsqlarr['uid']);
    						//工作经历
    						$worksqlarr['uid'] = $adduserinfo["uid"];
    						$worksqlarr['pid'] = $resumeResult["id"];
    						$worksqlarr['companyname'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AA".$i)->getValue());
    						$worksqlarr['jobs'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AB".$i)->getValue());
    						$worksqlarr['startyear'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AC".$i)->getValue());
    						$worksqlarr['startmonth'] = (string)trimall($objPHPExcel->getActiveSheet()->getCell("AD".$i)->getValue());
    						...
    						$faileList["resumeErrorlist"][] =$resumeError;
    					}
    				
    					
    				}else{
						$userinfo = ["errorinfo" =>$userinfo,info=>$adduserinfo ];
    		
    					$faileList["userErrorlist"][] = $userinfo;
    					$f++;//失败数
    				}
				}
    		}
     ...
     **创建简历-excel
     */
    public function resume_addall($data,$userinfo){
    	...
    	if($data){
    		...
    		foreach ($ints as $val) {
    			...
    			$setsqlarr[$val] =intval($data[$val]);
    		}
    		foreach ($trims as $val) {
    			//$setsqlarr[$val] = I('post.'.$val,'','trim,badword');
    			$setsqlarr[$val] = trim($data[$val]);
    		}
    		$resume_count == 0 && $setsqlarr['def'] = 1;
  //var_dump($setsqlarr);exit;
    		$rst=D('Resume')->add_resume($setsqlarr,$userinfo);
    		//var_dump($rst);exit;
    		//echo D('Resume')->getLastSql();exit;
    		if(!$rst['state']){
    			return $result = ["success" => 0,"error" => $rst['error'] ,'id'=>$rst['id']];
    		}else{
    			return $result = ["success" => 1,"error" => "" ,'id'=>$rst['id']];
    		}
    	}
    }
     ...
     */
    public function addallmember($userdata) {
    	...
    	if($userdata){
    		//var_dump($userdata);exit;
    		if(false === $data = $mod->create($userdata)){

    			IS_AJAX && $this->ajaxReturn(0,$mod->getError());
    			return ['success' => 0,'errorinfo'=>$mod->getError(),'mobile'=>$userdata['mobile']];
    		
    		}
    		
    		if(method_exists($this,'_before_insert')) {
    			$data = $this->_before_insertall($data);
    		}
    		//var_dump($data);exit;
    		if($id = $mod->add($data)){
    			if(method_exists($this,'_after_insert')){
    				$data[$pk] = $id;
    				$this->_after_insertall($id,$data);
    			}
    			//统一写日志
    			$this->admin_write_log_unify($id);
    			//echo 1;exit;
    			return ['id'=>$id,'success' => 1,'errorinfo'=>$mod->getError(),'mobile'=>$userdata['mobile']];
    		}
    	}
    }
    
    public function _before_insertall($data){
    	if(fieldRegex($data['username'],'number')){
    		return ['error' => '用户名不能是纯数字！','mobile'=>$data['mobile']];
    	
    	}
    	$data['password'] = D('Members')->make_md5_pwd($data['password'],$data['pwd_hash']);
    	return $data;
    }
    public function _after_insertall($id,$data){
    ...
    */
    ...
    public function ajax_get_interest_jobs($type=''){
        ...
        if($type=='recommend_jobs'){
            $jobcategory = M('Resume')->where(array('uid'=>C('visitor.uid')))->order('refreshtime desc')->getField('intention_jobs_id');
            $where['职位分类'] = $jobcategory;
            $where['排序'] = 'stickrtime';
            $msg = "没有合适的推荐职位！";
        }elseif($type=='nearby_jobs'){
            $where['经度'] = I('get.lng',C('qscms_map_center_x'),'trim,badword');//112.732929
            $where['纬度'] = I('get.lat',C('qscms_map_center_y'),'trim,badword');//37.714684
            $msg = "没有找到附近的职位！";
        }else{
            $where['排序'] = 'rtime';
            $msg = "没有找到最新的职位！";
    ...
    public function resume_edit(){
        ...
        if(IS_POST){
            ...
            if(1 == $auto_apply=I('post.auto_apply',0,'intval')){// 3天内 自动投递
                ...
                D('Resume')->where(array('id'=>$resume['id']))->setfield('entrust',3);
            }
        ...
        $auto_refresh=D('QueueAutoRefresh')->where(array('pid'=>$pid))->find();
		$auto_apply=D('ResumeEntrust')->where(array('resume_id'=>$pid))->find();
        $resume['tag_key'] = $resume['tag']?explode(',',$resume['tag']):array();
        $resume['tag_cn'] = $resume['tag_cn']?explode(',',$resume['tag_cn']):array();
		$start=$auto_refresh['refreshtime']-3*24*60*60;
    ...
    */
    public function ajax_resume_img_del(){
        if(IS_POST){
            ...
            if(strpos($row['img'], '..') !== false) die('Error Img.'); @unlink(C('qscms_attach_path')."photo/".$row['img']);
