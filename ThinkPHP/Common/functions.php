...
function U($url='',$vars='',$suffix=true,$domain=false,$type=false,$module_type=false) {
    ...
    // URL组装
    $depr       =   C('URL_PATHINFO_DEPR');
    $urlCase    =   C('URL_CASE_INSENSITIVE') && !$module_type;
    if($url) {
    ...
    if(C('URL_MODEL') == 0 || strtolower($var['c']) == 'admin' || $type) { // 普通模式URL转换
        $url = (strpos(__APP__,'/index.php') || strpos(__APP__,'/install.php') || __APP__ == '/index.php' || __APP__ == '/install.php') ? __APP__ : __APP__ . '/index.php';
        $url       .=   '?'.C('VAR_MODULE')."={$module}&".http_build_query(array_reverse($var));
        if($urlCase){
            $url    =   strtolower($url);
        }        
        if(!empty($vars)) {
        ...
        if(isset($route)) {
            ...
            $url    =   __APP__.'/'.($module && !$module_type?$module.MODULE_PATHINFO_DEPR:'').implode($depr,array_reverse($var));
        }
        if($urlCase){
            $url    =   strtolower($url);
        }
...
function F($name, $value='', $path=DATA_PATH) {
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            Think\Storage::put($filename,strip_whitespace("<?php\treturn " . var_export($value, true) . ";\n?>"),'F');
            // 缓存数据
            $_cache[$name]  =   $value;
            return null;
        }
    ...
    if (Think\Storage::has($filename,'F')){
        $value      =   Think\Storage::get_array($filename,'F');
        $_cache[$name]  =   $value;
    } else {
        $value          =   false;
    }
