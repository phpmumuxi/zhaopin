...
//cookie拦截规则
$cookiefilter = "\\b(and|or)\\b\\s*?([\\(\\)'\"\\d]+?=[\\(\\)'\"\\d]+?|[\\(\\)'\"a-zA-Z]+?=[\\(\\)'\"a-zA-Z]+?|>|<|\s+?[\\w]+?\\s+?\\bin\\b\\s*?\(|\\blike\\b\\s+?[\"'])|\\/\\*.+?\\*\\/|<\\s*script\\b|\\bEXEC\\b|UNION.+?SELECT|UPDATE.+?SET|INSERT\\s+INTO.+?VALUES|INTO.+?FILE|(SELECT|DELETE).+?FROM|(CREATE|ALTER|DROP|TRUNCATE)\\s+(TABLE|DATABASE)";
//referer获取
...
<html lang="en">
  <head>
    ...
    <meta name="author" content="逸管家智能数据">
      ...
      <form action="" method="post" class="form-signin">
		<fieldset>
			...
			<div class="row-fluid wukong">
				<div class="span3"><img style="height:50px;" src="./Public/img/logo_img.png" alt="逸管家智能数据"/></div>
				<div class="span9">
					<p>逸管家智能数据</p>
				</div>
			</div>
