...
class Model {
     ...
     */
    protected function identicalNull($data){
        $table = str_replace(C('DB_PREFIX'),'',$this->getTableName());
        foreach($data as $key=>$val){
            if(!$val) return $table.'_null_error_'.$key;
        }
        return true;
    }
    /**
     ...
     * @return Boolean/string 验证结果(true:合法,false:不合法,字符串：不合法提示语)
     */
    protected function identicalEnum($data){
        $table = str_replace(C('DB_PREFIX'),'',$this->getTableName());
        foreach($data as $key=>$val){
            if(preg_match('/^\d+$/',$val) !== 1) return $table.'_enum_error_'.$key;
        }
        return true;
    }
    public function addAll($dataList,$options=array(),$replace=false){
        ...
        if(false !== $result ) {
            ...
            if($insertId) {
                $c = count($dataList);
                if(false === $this->_addAll_after_insert($dataList,$options,$c)) {
                    return false;
                }
                return $c;
            }
    ...
    public function addAll1($dataList,$options=array(),$replace=false){
        if(empty($dataList)) {
            $this->error = L('_DATA_TYPE_INVALID_');
            return false;
        }
            ...
            foreach ($_auto as $auto){
                ...
                if( $type == $auto[2] || $auto[2] == self::MODEL_BOTH) {
                    ...
                    switch(trim($auto[3])) {
                        ...
                        case 'string':
                        default: // 默认作为字符串填充
                            !isset($data[$auto[0]]) && $data[$auto[0]] = $auto[1];
                    }
                ...
                if(empty($val[5]) || ( $val[5]== self::MODEL_BOTH && $type < 3 ) || $val[5]== $type ) {
                    ...
                    switch($val[3]) {
                        ...
                        default:    // 默认表单存在该字段就验证
                            $keys = explode(',',$val[0]);
                            $v = '';
                            foreach ($keys as $_val) {
                                if(isset($data[$_val])) $v.=','.$_val;
                            }
                            if($v){
                                $val[0] = ltrim($v,',');
    ...
    protected function _validationField($data,$val) {
        ...
        if(true !== $reg = $this->_validationFieldItem($data,$val)){
            if(false !== $reg) $val[2] = L(trim($reg));
    ...
    protected function getLang(){
        $table = str_replace(C('DB_PREFIX'),'',$this->getTableName());
        if (is_file(LANG_PATH.C('DEFAULT_LANG').'/'.$table.'.php')){
            L(include LANG_PATH.C('DEFAULT_LANG').'/'.$table.'.php');
        }
        if (is_file(MODULE_PATH.'Lang/'.C('DEFAULT_LANG').'/'.$table.'.php')){
            L(include MODULE_PATH.'Lang/'.C('DEFAULT_LANG').'/'.$table.'.php');
        }
    }
}