...
class Qscms extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'jobs_list' => array('attr'=>'type,搜索类型,显示数目,开始位置,职位名长度,企业名长度,描述长度,填补字符,应届生职位,职位分类,职位大类,职位小类,地区分类,地区大类,地区小类,道路,写字楼,标签,行业,学历,工作经验,工资,职位性质,公司规模,紧急招聘,推荐,关键字,关键字type,日期范围,排序,分页显示,会员UID,公司页面,职位页面,列表页,合并,公司列表名,公司职位页面,单个公司显示职位数,浏览过的职位,风格模板,列表名,去除id,经度,纬度,半径,搜索范围,联系方式','close'=>0),
        'company_jobs_list' => array('attr'=>'type,显示数目,开始位置,企业名长度,填补字符,地区大类,地区小类,紧急招聘,日期范围,推荐,职位名长度,显示职位,职位页面,职位分类,行业,排序,分页显示,公司页面,列表页,统计职位,列表名','close'=>0),
    ...
    private static function config_seo() {
        ...
        return 'array("pname"=>"'.$page['pname'].'","title"=>"'.$page['title'].'","keywords"=>"'.$page['keywords'].'","description"=>"'.$page['description'].'","header_title"=>"'.$page['title'].'")';
    }
    /**
     * 转换数据为HTML代码
     * @param array $data
